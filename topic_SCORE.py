...
def topic_score(word_doc, K_topic, K_cand, n_cluster, word_freq_quantile = 1, kmeans_start = 200):
    ...
    V = vertices.V
    theta = vertices.theta

    # Step 3
    R_extend = np.concatenate((np.ones(n,1), R), axis = 1)
    V_extend = np.concatenate((np.ones(n,1), V), axis =1)
    Pi = R_extend @ np.linalg.solve(V_extend)
    Pi = np.maximum(Pi, 0)
...
def vertices_estimate(R, K_cand, n_cluster, kmeans_start):
    ...
    theta0 = theta[top2,:]
    theta = theta[np.setdiff1d(np.arange(n),top2),:]
    ...
    if K_cand > 2:
        for k0 in range(3, K_cand + 1):
            ...
            theta0 = np.concatenate((theta0, theta[max_ix,:]), axis = 0)
            theta = theta[np.setdiff1d(np.arange(n_k0),max_ix),:]
        theta = theta0

    # Step 2b
    index_matrix = np.array(list(itertools.combinations(np.arange(K_cand),K))) # K_topic x (K_cand choose K_topic) array
...
def simplex_distance(x, V):
    ...
    G = np.concatenate((np.eye(n_v - 1), -np.ones(n_v - 1)), axis = 1)
    VV = G @ V
    P = VV @ VV.T
    q = VV @ (x - V[nv - 1, :]).reshape((p,1))
    h = np.zeros(n_v)
    h[nv - 1] = -1

    obj = qp_const(P = P, q = q, G = G, h = h)
    return np.sum((x - V[nv-1,:])**2) + 2 * obj
